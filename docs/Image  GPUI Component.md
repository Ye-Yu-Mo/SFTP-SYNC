---
title: "Image | GPUI Component"
source: "https://longbridge.github.io/gpui-component/docs/components/image"
author:
published:
created: 2025-12-05
...
```rust
rustuse gpui::{img, ImageSource, ObjectFit};
...
```rust
rust// Cover - scales to fill container, may crop
...
```
```rust
rust// Image with loading skeleton
...
```rust
rust// SVG icon with custom styling
...
```rust
rustuse gpui_component::{v_flex, div, Icon, IconName};
...
```
```rust
rustfn custom_avatar(src: &str, name: &str, size: f32) -> impl IntoElement {
...
```
```rust
rustfn image_comparison(before_src: &str, after_src: &str) -> impl IntoElement {
