---
title: "Popover | GPUI Component"
source: "https://longbridge.github.io/gpui-component/docs/components/popover"
author:
published:
created: 2025-12-05
...
```rust
rustuse gpui_component::popover::{Popover};
```
...
Any element that implements [Selectable](https://docs.rs/gpui-component/latest/gpui_component/trait.Selectable.html) can be used as a trigger, for example, a [Button](https://docs.rs/gpui-component/latest/gpui_component/button/struct.Button.html).

Any element that implements [RenderOnce](https://docs.rs/gpui/latest/gpui/trait.RenderOnce.html) or [Render](https://docs.rs/gpui/latest/gpui/trait.Render.html) can be used as popover content, use `.child(...)` to add children directly.

```rust
rustuse gpui::ParentElement as _;
...
You can add any `Entity<T>` that implemented [Render](https://docs.rs/gpui/latest/gpui/trait.Render.html) as the popover content.

```rust
rustlet view = cx.new(|_| MyView::new());
...
This `content` callback will called every time on render the popover. So, you should avoid creating new elements or entities in the content closure or other heavy operations that may impact performance.
...
```rust
rustuse gpui_component::{DismissEvent, popover::Popover};
...
And the `Popover` has implemented the [Styled](https://docs.rs/gpui/latest/gpui/trait.Styled.html) trait, so you can use all the styling methods provided by GPUI to style the popover content as you like.

```rust
rust// For custom styled popovers or when you want full control
...
When you use `open` to control the popover's open state, that means you have take full control of it, so you need to update the state in `on_open_change` callback to keep the popover working correctly.
