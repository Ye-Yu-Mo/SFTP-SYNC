---
title: "Label | GPUI Component"
source: "https://longbridge.github.io/gpui-component/docs/components/label"
author:
published:
created: 2025-12-05
...
```rust
rustuse gpui_component::label::{Label, HighlightsMatch};
```
...
```rust
rustLabel::new("This is a label")
```

### Label with Secondary Text

```rust
rust// Label with optional indicator
...
```rust
rust// Full text highlighting (finds all matches)
...
```rust
rustuse gpui_component::green_500;
...
```
```rust
rust// For sensitive information
...
```rust
rust// Text wrapping with line height
...
```rust
rust// Using text size utilities
...
```rust
rust// Interactive search highlighting
...
```rust
rust// Financial data with toggle
