---
title: "NumberInput | GPUI Component"
source: "https://longbridge.github.io/gpui-component/docs/components/number-input"
author:
published:
created: 2025-12-05
...
```rust
rustuse gpui_component::input::{InputState, NumberInput, NumberInputEvent, StepAction};
```
...
```rust
rustlet number_input = cx.new(|cx|
...
```rust
rust// Integer input with validation
...
```rust
rustuse gpui_component::input::MaskPattern;
...
```rust
rustuse gpui_component::{button::{Button, ButtonVariants}, IconName};
...
```rust
rustNumberInput::new(&input).disabled(true)
```

### Without Default Styling

```rust
rust// For custom container styling
...
```rust
rustlet number_input = cx.new(|cx| InputState::new(window, cx));
...
```rust
rust// Increment programmatically
