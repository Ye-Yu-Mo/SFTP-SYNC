---
title: "WebView | GPUI Component"
source: "https://longbridge.github.io/gpui-component/docs/components/webview"
author:
published:
created: 2025-12-05
...
gpui-component = { version = "0.5.0-preview2", features = ["webview"] }
```

## Import

```rust
rustuse gpui_component::webview::WebView;
...
```rust
rust// Configure WebView with custom settings
...
```rust
rust// WebView automatically handles mouse events within its bounds
...
```rust
rust// Access WRY methods directly
...
```rust
rustpub fn init_webview(_: &mut App) {
...
```rust
rustfn load_safe_url(webview: &mut WebView, url: &str) -> Result<(), &'static str> {
...
```rust
rustfn execute_safe_script(webview: &mut WebView, script: &str) -> anyhow::Result<()> {
...
```rust
rust// Only render webview when needed
